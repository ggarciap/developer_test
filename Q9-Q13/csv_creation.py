...
import csv
DB_HOST = 'tradewater-test.c8fgzcvr9wmy.us-west-2.rds.amazonaws.com'
DB_USER = 'test_taker'
DB_PORT = 3306
QUERY='''
    WITH CTE1 AS 
...
try:
    db=MySQLdb.connect(host=DB_HOST,user=DB_USER, port=DB_PORT)
    cur=db.cursor()
    cur.execute(QUERY)
    result=cur.fetchall()
except MySQLdb.err as err:
    print(err)
    exit(1)

column_names = [i[0] for i in cur.description]
c= csv.writer(open('Q9-output.csv', 'w', encoding='utf-8'))
