...
Recipient_Count AS (
	SELECT CTE2.gender, COUNT(CTE2.mailing_recipient_id) AS recipient_count
	FROM CTE2
	GROUP BY CTE2.gender 
),
Opens_count AS (
...
Opens_per_recipient AS (
	SELECT CTE2.gender , COUNT(cr.mailing_recipient_id) AS opens_per_recipient
...
Clicks_per_recipient AS (
	SELECT CTE2.gender, COUNT(cr.mailing_recipient_id) AS clicks_per_recipient
	FROM CTE2 INNER JOIN tradewater.mailing_recipient_click cr ON CTE2.mailing_recipient_id = cr.mailing_recipient_id 
	WHERE cr.mailing_recipient_click_type_id = 1
	GROUP BY CTE2.gender 
),
...
Actions_per_recipient AS (
	SELECT CTE3.gender , COUNT(CTE3.cons_action_id) AS actions_per_recipient
	FROM CTE3
	GROUP BY CTE3.gender 
),
Combined_recipient AS(
	SELECT rc.gender, recipient_count, opens_per_recipient, clicks_per_recipient, actions_per_recipient
	From Recipient_Count rc 
		INNER JOIN Opens_per_recipient opr 
			ON rc.gender = opr.gender
		INNER JOIN Clicks_per_recipient cpr
			ON cpr.gender = opr.gender
		INNER JOIN Actions_per_recipient napr
			ON napr.gender = cpr.gender
)
SELECT cr.gender, cr.recipient_count, cr.opens_per_recipient, oc.opens_count, cr.clicks_per_recipient, cc.click_count, cr.actions_per_recipient, na.number_of_actions
